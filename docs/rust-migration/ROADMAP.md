...
### 登入導向與 VVIP 預覽強化(2026-09-03)

**登入後落地頁**：帳密登入原導向 `Home`、Google 經 `Worker → /dashboard → /profile`，對剛完成驗證的會員皆是死路（首頁資訊未因身份展開、個人檔案是編輯表單）。現依 `AuthUser.membership_tier` + `is_verified` 分流：`Diamond/Black Card + 已驗證` 直達 `/vvip`，其餘到 `/events`（精選活動）—— 後者在 `is_authenticated` 時已多揭露價格、名額與報名狀態，符合「登入後精選活動應有更多資訊揭露」。

**管理站上方分頁**：`shell.rs` 的 `PRIMARY_NAV_ITEMS` 原為會員站三項且雙 bundle 共用，導致在 `/admin` 仍看到首頁/精選活動/VVIP。現依 `feature = "admin-bundle"` 分流，`admin` bundle 的 top nav 為 `管理後台 / 活動管理 / 銷售管理 / 系統健康 / 會員站`。

**VVIP 預覽**：`PreviewAttendee` 新增 `gender` 與 `profile_picture`（解析 `profilePicture`/`profile_picture`），卡片改為真實美照（`media.ahexagram.com` 有值時 `<img>`，無值時皇冠占位）＋等級徽章＋已驗證勾選，`hover:scale` 與 `hs-enter`。`users` 新增可空 `gender` / `interested_in`（`female/male/non_binary/everyone/prefer_not_to_say`），`profile` 編輯頁新增兩組選單，預覽以 `viewer.interested_in` 對 `attendee.gender` 篩選（空或 `prefer_not_to_say` 視為對所有人可見），用優質會員吸引優質會員且尊重取向。
