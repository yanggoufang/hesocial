      ...
      <div className="space-y-6">
        <div className="luxury-glass p-4 rounded-lg">
          <div className="flex flex-col md:flex-row justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-luxury-platinum mb-4 md:mb-0">Categories</h2>
            ...
            <table className="w-full text-sm text-left text-luxury-platinum/80">
              ...
              <tbody>
                {filteredCategories.map((category) => (
                  <tr key={category.id} className="border-b border-luxury-gold/10 hover:bg-luxury-gold/5">
                    <td className="px-6 py-4 font-medium text-white">{category.name}</td>
                    <td className="px-6 py-4">{category.slug}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${category.is_active ? 'bg-green-500/20 text-green-300' : 'bg-gray-500/20 text-gray-300'}`}>
                        {category.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right space-x-2">
                      <button className="luxury-button-icon"><Edit className="h-4 w-4" /></button>
                      <button onClick={() => { setSelectedCategory(category); setShowDeleteModal(true); }} className="luxury-button-icon-danger"><Trash2 className="h-4 w-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="luxury-glass p-6 rounded-lg text-center w-full max-w-md">
            <h3 className="text-lg font-bold text-luxury-gold mb-4">Create New Category</h3>
            <input
              type="text"
      ...
      {showDeleteModal && selectedCategory && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="luxury-glass p-6 rounded-lg text-center">
            <h3 className="text-lg font-bold text-luxury-gold mb-4">Delete Category</h3>
